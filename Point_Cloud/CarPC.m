function [FscoreCar, spDivRIr, PercCar] = CarPC

%% load data
load TrainVal_Elimina_MIN_MAX_RI_PC;
load TrainVal_labelsPC;
load TrainVal_predictPC;

%% Car
carIndex = TrainVal_labelsPC == 1; % binary index for car labels
dmCar = TrainVal_Elimina_MIN_MAX_RI_PC(carIndex);
...
dmMIN = spDivRI(i);
dmMax = spDivRI(i+1);
pilarIndex = (dmCar >= dmMIN) & (dmCar < dmMax);
