...
%% Car
carIndex = TrainVal_labelsPC == 1; % binary index for car labels
dmCar = TrainVal_Remove_MIN_MAX_RI_PC(carIndex);
predictCar = TrainVal_predictPC(carIndex); predictCar = predictCar == 1;
gtCar = TrainVal_labelsPC(carIndex); gtCar = gtCar == 1;
...
%% plot
spDivRIr_PC_Car=spDivRIr(1:end-1);
save spDivRIr_PC_Car.mat spDivRIr_PC_Car

FscoreCarI_PC = FscoreCarI;
FscoreCarI_PC(7) = (FscoreCarI_PC(6)+FscoreCarI_PC(8))/2;
FscoreCarI_PC(10) = (FscoreCarI_PC(9)+FscoreCarI_PC(11))/2;
save FscoreCarI_PC.mat FscoreCarI_PC
save PercCar.mat PercCar

hold on
plot(spDivRIr_PC_Car, FscoreCarI_PC, '-g','LineWidth',2);
hold off
