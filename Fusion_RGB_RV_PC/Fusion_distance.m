...
%% Determinando a distância para cada amostra usando DM
x_min = 5;   % movement direction
x_max = 80;
load('Test_Remove_MIN_MAX_RI_DM.mat');
test_Remove_MIN_MAX_RI_DM = double(Test_Remove_MIN_MAX_RI_DM);
dm = test_Remove_MIN_MAX_RI_DM./255;
DM = -x_max*x_min./(dm.*(x_max-x_min)-x_max);
DM(vazio)=[];

%%
Test_ProbabilityRGB(vazio,:)=[];
Test_ProbabilityDM(vazio,:)=[];
Test_ProbabilityPC(vazio,:)=[];
load Test_labels.mat
Test_labels(vazio,:)=[];
%% PC: Determinar o valor da ponderação para cada amostra usando interpolação.
load('Normalized_PC')
load('spDivRIr_DM_PC_Car')
wpc = zeros(size(DP,1),1);
D=DP;
for k=1:size(D,1)
    k;
    menor_distance = spDivRIr_DM_PC_Car(D(k)>=spDivRIr_DM_PC_Car);
    menor_distance = menor_distance(end);
    
    menor_pond = find(spDivRIr_DM_PC_Car==menor_distance);
    % menor_pond = Normalized_DM_PC(menor_pond(end));
    menor_pond = Normalized_PC(menor_pond(end));

    if D(k)>max(spDivRIr_DM_PC_Car)
        maximo_distance = 80;
        maximo_pond = 0;
    else    
        maximo_distance = spDivRIr_DM_PC_Car(D(k)<spDivRIr_DM_PC_Car);
        maximo_distance = maximo_distance(1);
        maximo_pond = find(spDivRIr_DM_PC_Car==maximo_distance);
        % maximo_pond = Normalized_DM_PC(maximo_pond(1));
...
%%
D=[];
%% DM: Determinar o valor da ponderação para cada amostra usando interpolação.
load('Normalized_DM')
load('spDivRIr_DM_PC_Car')
wdm = zeros(size(D,1),1);
D=DM;
for k=1:size(D,1)
    k;
    menor_distance = spDivRIr_DM_PC_Car(D(k)>=spDivRIr_DM_PC_Car);
    ...
    wdm(k)=menor_pond+(maximo_pond-menor_pond).*((D(k)-menor_distance)/(maximo_distance-menor_distance));
end

%% Calculando a fusão dos scores
Score_final =zeros(size(wdm,2),3);
for j=1:size(wdm,2)
     Score_final(j,:) = (1-wpc(j)-wdm(j)).*Test_ProbabilityRGB(j,:)+(wpc(j)).*Test_ProbabilityPC(j,:)+(wdm(j)).*Test_ProbabilityDM(j,:);
end
